...
def sample_xyz_rgb_from_textured_mesh(mesh, n=10000):
    """
    Returns:
      xyz: (n, 3)
      rgb: (n, 3) in [0,1]
    """
    assert len(mesh) == 1
    # Many versions return (xyz, rgb) when return_textures=True
    out = sample_points_from_meshes(mesh, n, return_textures=True)

    if isinstance(out, (tuple, list)) and len(out) == 2:
        xyz, rgb = out
    else:
        raise RuntimeError(
            "Your PyTorch3D sample_points_from_meshes doesn't return textures. "
            "Use Option B below."
        )

    return xyz[0], rgb[0][..., :3]  # rgb may be (n, C); take first 3
...
class Deform_Model(nn.Module):
    def __init__(self, device, mesh_dir, k=10, num_clusters=5000, d_pe=64, expre_dir=None, eigen_num=None, normalize_mesh=False, normalize_scale=False, s=1, tx=0, ty=0, tz=0):
        ...
        samples, samples_rgb = sample_xyz_rgb_from_textured_mesh(self.mesh, n=self.num_samples)
        self.geometry_shape = torch.cat((self.geometry_shape, samples), dim=0)
        self.verts_colors   = torch.cat((self.verts_colors, samples_rgb), dim=0)
